...
export default async function refreshYoutube() {
   ...
   for (const [index, item] of youtubeChannels.entries()) {
      try {
         ...
         const feedData = await getFeedData(`https://www.youtube.com/feeds/videos.xml?channel_id=${item.uuid}`, "youtube");
         ...
         if (Array.isArray(feedData.entry)) {
            ...
            entries = [{
               ...
               link: feedData.entry.link["@href"] || `https://obradoirodixitalgalego.gal/comunidade/proxectos/${item.id}/`,
               youtube_id: item.id
            }]
         }
         ...
         logger.info(`${index + 1}/${youtubeChannels.length}`, `Recuperados ${entries.length} vídeos de ${item.title}.`);
         // Rate Limit 1/sec
